// app/HomeClient.tsx
// Same design as before — now reads content from Sanity instead of hardcoded text.
...
// Used only if she hasn't published anything in the Studio yet,
// so the site never shows up broken or blank.
const FALLBACK = {
  heroTag: "Nettoyage professionnel à Annecy",
  heroTitleLine1: "Les Alpes",
  heroTitleLine2: "Multiservices",
  heroSubtitle:
    "Des prestations de nettoyage haut de gamme pour immeubles, bureaux, Airbnb et fins de chantier.",
  phone: "+33767575659",
  email: "alpesmultiservices@icloud.com",
  addressLine1: "4 rue des Tisserands",
  addressLine2: "Cran-Gevrier, Annecy (74960)",
  services: [
    {
...
export default function HomeClient({ data }: { data: SiteSettings }) {
  const content = {
    heroTag: data?.heroTag || FALLBACK.heroTag,
    heroTitleLine1: data?.heroTitleLine1 || FALLBACK.heroTitleLine1,
    heroTitleLine2: data?.heroTitleLine2 || FALLBACK.heroTitleLine2,
    heroSubtitle: data?.heroSubtitle || FALLBACK.heroSubtitle,
    phone: data?.phone || FALLBACK.phone,
    email: data?.email || FALLBACK.email,
    addressLine1: data?.addressLine1 || FALLBACK.addressLine1,
    addressLine2: data?.addressLine2 || FALLBACK.addressLine2,
    services:
      data?.services && data.services.length > 0
        ? data.services
        : FALLBACK.services,
    gallery:
      data?.gallery && data.gallery.length > 0 ? data.gallery : FALLBACK.gallery,
  };

  const telHref = `tel:${content.phone}`;
  const telDisplay = content.phone.replace(
    ...
    <main className="bg-white text-slate-800 overflow-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center">
        <div
          ...
          style={{
            backgroundImage: `url('${content.gallery[0].url}')`,
          }}
        />
      ...
      <section className="relative -mt-10 z-20 px-6">
        <div className="max-w-5xl mx-auto bg-white rounded-3xl shadow-2xl border border-slate-100">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-8">
            {[
              { icon: Clock3, title: "24h/24", subtitle: "Intervention rapide" },
              {
                icon: ShieldCheck,
                title: "Fiable",
                subtitle: "Professionnalisme garanti",
              },
              { icon: Star, title: "Qualité", subtitle: "Résultats impeccables" },
              {
                icon: Sparkles,
                title: "Premium",
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-black tracking-tight">
              ...
              <span className="block text-slate-400">pour chaque espace</span>
            </h2>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {content.services.map((service, index) => {
              const Icon = iconByType[service.type] ?? Building2;
              return (
                <motion.div
                  whileHover={{ y: -8 }}
                  key={index}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {content.gallery.map((img, index) => (
              ...
              >
                <img
                  src={img.url}
                  alt={img.caption || "Nettoyage professionnel"}
                ...
                <div className="absolute bottom-6 left-6 text-white">
                  ...
                  <h3 className="text-2xl font-bold">{img.caption}</h3>
                </div>
              </motion.div>
            ))}
          </div>
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-600">{telDisplay}</p>
                </div>
              </div>
      ...
      <footer className="border-t border-slate-200 py-10 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="font-black text-xl">Les Alpes Multiservices</h3>
